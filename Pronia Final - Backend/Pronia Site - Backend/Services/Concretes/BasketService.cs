            ...
            List<BasketCookieViewModel> AddCookie()
            {
                ...
                if (cookieViewModel is null)
                {
                    productCookieViewModel.Add
                        (new BasketCookieViewModel(product.Id, product.Name, product.Images.Take(1).FirstOrDefault() != null
                        ? _fileService.GetFileUrl(product.Images.Take(1).FirstOrDefault().ImageNameInFileSystem, Contracts.File.UploadDirectory.Product)
